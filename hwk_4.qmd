...
locations_by_eez <- merge(oyster_eez, wc_eez_rast) |> 
  group_by(rgn) |> 
  summarise(elevation = sum(suitable == 1, na.rm = TRUE))
...
merged_rast <- rasterize(merged_df)
# probably want to replot this to show suitable locations and eezs
```
